...
import type { MovieCreate, MovieDetail, MovieFilterOptions, MoviePage, MoviePatch } from './types'

export interface MovieFilters { page: number; page_size: number; q?: string; ano?: number; genero?: string; poster_first?: boolean }
...
export const createMovie = (payload: MovieCreate, token: string) =>
  request<MovieDetail>('/movies', { method: 'POST', body: JSON.stringify(payload) }, token)

export const patchMovie = (id: string, payload: MoviePatch, token: string) =>
  request<MovieDetail>(`/movies/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(payload) }, token)

export const deleteMovie = (id: string, token: string) =>
  request<void>(`/movies/${encodeURIComponent(id)}`, { method: 'DELETE' }, token)
